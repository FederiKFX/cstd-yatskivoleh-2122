...
def gameplay():
    ...
    while not gameQuit:
        ...
        while not gameOver:
            ...
            if (mode == 1 and len(pteras) == 0 and random.randrange(0,200) == 10 and counter > 500) or pter == 1:
                if pter == 1:
                    last_obstacle.empty()
...
def main():
    Thread(target = GameWork, daemon = False).start()
    Thread(target = COMPortWork, daemon = False).start()
